...
if __name__ == "__main__":
    ...
    if args.word:
        if not os.path.exists(args.data_word_model):
            built_curpus(train_text, args.embedding_num)
        model_path = [args.save_word_model_best, args.save_word_model_last]
    else:
        if not os.path.exists(args.data_words_model):
            built_curpus(train_text, args.embedding_num)
        model_path = [args.save_words_model_best, args.save_words_model_last]
    ...
    print(f"test dataset accuracy:{accuracy:.4f}\tprecision:{precision:.4f}\trecall:{recall:.4f}\tf1:{f1:.4f}")
